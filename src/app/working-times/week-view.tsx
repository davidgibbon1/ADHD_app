...
export default function WeekView({ userId, onHasUnsavedChanges }: WeekViewProps) {
  ...
  const loadTimeBlocks = async () => {
    try {
      ...
      const response = await fetch(`/api/time-blocks?userId=${userId}&isIdealWeek=false`);
  ...
  const goToPreviousWeek = () => {
    if (hasUnsavedChanges && !confirm('You have unsaved changes. Are you sure you want to change weeks without saving?')) {
      return;
    }
  ...
  const goToToday = () => {
    if (hasUnsavedChanges && !confirm('You have unsaved changes. Are you sure you want to go to today without saving?')) {
      return;
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (saveResponse.ok) {
        setSaveMessage('Weekly schedule saved successfully!');
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={handleSave}
            className={`ml-4 ${hasUnsavedChanges ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-purple-600 hover:bg-purple-700'} text-white`}
          >
      ...
      <p className="text-white/70">
        Plan your current week with time blocks for specific activities and projects.
        Click and drag on the calendar to create time blocks.
      </p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="E.g., Meeting, Workout, Project Work"
